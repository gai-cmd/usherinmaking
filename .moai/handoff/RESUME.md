...
마지막 갱신: 2026-08-02 · 커밋 `67fcc9a`(저널 DB 배선 + 네이버 9건 취입) 이후 — 승격까지 완료

## 8/2 세션 — 촬영후기가 실제 원고로 바뀌었다

**배선이 먼저 문제였다.** 공개 저널 목록·상세가 `@/content/journal`(코드 시드)만 읽고 있어
DB에 넣어도 화면에 안 나오는 닫힌 고리였다. 이미지 슬롯·작품 그리드와 **같은 계열의 세 번째 누락**이다.
`src/server/journal-content.ts` 가 DB→화면 타입을 해석하고(DB 우선·시드 폴백·조회 실패 시 폴백),
`content/journal.ts` 헬퍼 6개에 `posts` 인자를 열어 두 페이지를 이었다. `generateStaticParams` 도 DB를 본다.

**네이버 9건 취입 완료** — 프로덕션 `/ko/journal` 에 실제로 떠 있다(실측).
`node scripts/naver-journal-import.mjs --limit N [--dry]` 로 더 넣을 수 있다.

취입 규칙(SEO 판단서 `DECISION-BLOG-IMPORT-SEO.md` 그대로): 제목 껍데기 제거 · 원문 링크 명시 ·
표지 Blob 재호스팅 · 본문 400자·이미지 3장 미만 제외 · KO 로케일만(JA·EN 은 시드 7건 유지).

**드라이런이 잡은 두 가지 (같은 실수 반복 금지)**
1. 자동 요약문이 거제 촬영 글에 "오키나와에서 진행한"을 붙였다 → **요약 생성 자체를 뺐다.**
   장소·촬영 종류를 코드가 단정하면 없는 사실을 만든다.
2. 지역 필터가 언급 유무만 봐서 거제 일정 공지가 통과해 실제로 DB에 들어갔다
   (오키나와 3회 vs 거제 13회) → 빈도 비교로 바꾸고 해당 글 삭제.

**아직 사람 손이 필요한 것 — 제목.** 자동 정리로는 한계가 있다. 현재 취입분 중
`오키나와여행에서 필수체트`(원문 오타), `오키나와스냅,오키나와가족스냅 — 오키나와여행추억`(키워드 잔재)
같은 제목이 그대로 떠 있다. 관리자 저널 편집 화면에서 고치는 것이 맞다.
...
- 나하공항에서 소요시간 · 구글맵 좌표 — **"크롤링 불가"는 방법이 틀렸던 것이다 (8/1 GPT 교차 감사).**
  지도 페이지를 긁을 필요가 없다. 단축링크의 리디렉션만 따라가면 최종 URL 의 `@위도,경도` 에
  좌표가 그대로 있다(`curl -sIL <단축링크> | grep -i location`, 키·로그인 불필요).
  **단 단축링크 자체가 저장소 어디에도 없다 — 그 링크만 다시 주시면 좌표는 바로 뽑는다.**
- 대표 이메일 · 로고 SVG · 작가 포트레이트 · 드레스 개별 컷
- ~~촬영후기 실제 원고 — 네이버가 외부 접근을 막아 미반영~~ → **틀린 판정이었다 (8/1 실측으로 뒤집음).**
  막힌 것은 `blog.naver.com/<id>` 뿐이고 그건 iframe 껍데기다. 아래 두 경로는 열려 있다:

  ```bash
  # ① 목록 50건 — 제목·날짜·링크·발췌(약 350자)·썸네일
  curl -s https://rss.blog.naver.com/usherinmaking.xml
  # ② 본문 전문 — 문단 13개·원본급 이미지 4장 (logNo 는 ①의 link 에서 딴다)
  ...
  **다만 아직 반영하지 않은 이유는 세 가지다** — ⑴ 원고가 한국어뿐이라 3개 언어 독립 본문
  ...
  그 화면이 OAuth 미발급으로 잠겨 있다. **즉 남은 것은 접근이 아니라 편집 방침 결정이다.**
