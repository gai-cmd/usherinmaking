...
미도달 1건: `gallery/empty`(결과 0건 화면 문구). 코드는 배선돼 있으나 0건이 되는 URL 조합을 만들지 못해
런타임으로 확인하지 못했다. 확인하려면 필터 URL 파싱(`selectionSegments`)이 실제로 만드는 selection 을 먼저 봐야 한다.
...
1. **Blob 실업로드** — 토큰이 빈 문자열이다. `vercel env pull` 로 실토큰을 받거나 dev 환경에 OIDC 를 켜야
   사진 교체를 사람이 손으로 확인할 수 있다. 사용자가 사진 전량 교체 예정이라 이게 제일 급하다
2. **관리자 로그인** — `AUTH_GOOGLE_ID` / `AUTH_SECRET` / `ADMIN_ALLOWED_EMAILS` 가 전부 비어 있어
   관리자 화면을 사람이 열어본 적이 없다. 서버 함수 단위로만 검증했다
3. **알림** — 문의 저장은 되지만 메일/LINE 은 아직 seam
