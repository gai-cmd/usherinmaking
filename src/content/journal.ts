...
export type JournalPost = {
  ...
  isSample?: true;
  /**
   * 얇은 글 표시 — 본문 글자 수가 기준에 못 미치는 취입 글.
   *
   * 화면에는 다른 글과 똑같이 나온다. 다르게 취급하는 곳은 검색엔진뿐이다:
   * 상세 페이지가 noindex 를 내보내고 사이트맵에서 빠진다. 사진 위주의 옛 블로그 글을
   * 원문 그대로 실으면서도 얇은 페이지 대량 발생으로 사이트 평가가 깎이지 않게 하는 장치다.
   * (원문을 고쳐 쓰는 대신 색인 신호만 닫는다 — 원문 불훼손이 사용자 요구다.)
   */
  thin?: boolean;
};
...
> = {
  ...
  ko: {
    eyebrow: 'JOURNAL',
    heading: '촬영후기',
    lead: ['촬영 당일의 날씨와 장소, 그날의 에피소드까지.', '스냅 촬영 전에 알아두면 좋은 이야기를 모았습니다.'],
    readMore: '이어서 읽기',
    related: '관련 글',
    planLead: '이 글의 촬영 플랜',
    planDetail: '플랜 상세',
    ...
    listMeta: {
      ...
      description:
        '촬영 당일의 날씨와 장소, 그날의 에피소드까지. 오키나와 스튜디오와 로케이션 촬영 전에 알아두면 좋은 이야기를 모았습니다.',
    },
  },
};
